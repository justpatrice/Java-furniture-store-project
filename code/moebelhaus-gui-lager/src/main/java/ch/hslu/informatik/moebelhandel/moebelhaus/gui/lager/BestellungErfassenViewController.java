...
public class BestellungErfassenViewController implements Initializable {
	...
	private static final String ERROR_MSG_EINGABE_ANZAHL_NICHT_KORREKT = "Die Eingabe für Anzahl ist nicht korrekt.";
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		try {
			...
			cmbLieferant.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<Lieferant>() {
				...
				@Override
				public void changed(ObservableValue<? extends Lieferant> observable, Lieferant oldValue,
						Lieferant newValue) {
					if (newValue != null) {
						updateCmbProduktname();
					}
			...
			cmbProduktname.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<ProduktTyp>() {
				...
				@Override
				public void changed(ObservableValue<? extends ProduktTyp> observable, ProduktTyp oldValue,
						ProduktTyp newValue) {
					if (newValue != null) {
						updateCmbProduktTypCode();
					}
			...
			/*
			 * Lieferant darf nicht geändert werden, wenn mindestens eine
			 * Bestllungsposition in der Tabelle existiert
			 */
			cmbLieferant.disableProperty().bind(Bindings.size(tblBestellung.getItems()).isNotEqualTo(0));

			/*
			 * Bestellung kann generiert werden, wenn mindestens eine
			 * Bestellungsposition enthalten ist
			 */
	...
	private void bestellungspositionHinzufuegen() {
		...
		try {
			...
			BestellungPosition bPos = new BestellungPosition(cmbProduktCode.getSelectionModel().getSelectedItem(),
					anzahl);

			/*
			 * Prüfen, ob evtl. ein ProduktTyp bestellt werden soll, für den
			 * eine Bestellungsposition bereits vorhanden ist. Falls ja, wird
			 * nur die Anzahl entsprechend angepasst.
			 */
			...
			if (index != -1) {
				/*
				 * Eine BestellungPosition für den ausgewählten ProduktTypCode
				 * existiert bereits und wird mit der zu erstellenden
				 * BestellungPosition gemergt.
				 */

				/* Anzahl berechnen (addieren) */
				int anzahlNeu = bPos.getAnzahl()
						+ tblBestellung.getItems().get(index).getBestellungPosition().getAnzahl();
				/*
				 * Die neue Anzahl in die neue BestellungPosition (bPos)
				 * übernehemen
				 */
				bPos.setAnzahl(anzahlNeu);

				/*
				 * Neue BestellungPositionWrapper Instanz erstellen, in der bPos
				 * enthalten sein wird
				 */
				BestellungPositionWrapper bPosWrapper = new BestellungPositionWrapper(tblBestellung.getItems().size(),
						bPos);
				...
				BestellungPositionWrapper bPosWrapper = new BestellungPositionWrapper(
						tblBestellung.getItems().size() + 1, bPos);
			...
			lblError.setText(ERROR_MSG_EINGABE_ANZAHL_NICHT_KORREKT);
			return;
		}
	}
	...
	private void bestellungGenerieren() {
		...
		try {
			...
			alert.setTitle("Bestellung generieren");
	...
	private void updateCmbProduktname() {
		...
		if (lieferant != null) {
			...
			try {
				List<ProduktTyp> produktTypListe = Context.getInstance().getMoebelhausLagerService()
						.findeByLieferant(lieferant);
	...
	private void updateCmbProduktTypCode() {
		...
		try {
			...
			List<ProduktTyp> produktTypListe = Context.getInstance().getMoebelhausLagerService()
					.findeByProduktname(cmbProduktname.getSelectionModel().getSelectedItem().getName());
