...
public class KasseManager implements KasseService {
	...
	public double getTagesAbrechnung(GregorianCalendar datum) throws Exception {
		try {
			List<Rechnung> rechnungen =  getRechnungDAO().findByDatum(datum);
			double result = 0.00;
			for(Rechnung rechnung : rechnungen){
				result += rechnung.getBetrag();
			}
			return result;
		} catch (Exception e) {
			String msg = "Keine Abtrechnung am Tag " + datum.toString() + " möglich"; 
			logger.error(msg, e);
			throw new Exception(msg);
		}
	}
